...
export const POST: APIRoute = async ({ request, cookies }) => {
    ...
    // Confirm delivery (works from 'incident' status per migration 20250511_cancel_incident.sql)
    ...
    if (itemsError || !orderItems) {
        console.error('failed to fetch order items for fund release (cancel-incident)', itemsError);
        return jsonResponse({ error: 'Incident cancelled but fund release failed' }, 500);
    }

...
export const POST: APIRoute = async ({ locals, request }) => {
    ...
    try {
        ...
        console.error(JSON.stringify({ event: 'catalog_upload.failed', error: error instanceof Error ? error.message : String(error) }));
...
export const DELETE: APIRoute = async ({ locals, request, url }) => {
    ...
    try {
        ...
        console.error(JSON.stringify({ event: 'catalog_delete.failed', error: error instanceof Error ? error.message : String(error) }));
        return new Response(JSON.stringify({ error: t.apiInternalError }), { status: 500 });
    }
};

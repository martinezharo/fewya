...
export const POST: APIRoute = async ({ locals, cookies, request  }) => {
    ...
    if (convexOnly) {
        ...
        try {
            ...
            console.error(JSON.stringify({ event: 'shop_upload.convex_failed', error: error instanceof Error ? error.message : String(error) }));
...
export const DELETE: APIRoute = async ({ locals, cookies, request, url  }) => {
    ...
    if (convexOnly) {
        ...
        try {
            ...
            console.error(JSON.stringify({ event: 'shop_delete.convex_failed', error: error instanceof Error ? error.message : String(error) }));
            return new Response(JSON.stringify({ error: t.apiInternalError }), { status: 500 });
        }
    }

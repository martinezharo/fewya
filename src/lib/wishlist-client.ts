...
function getHeartSvg(filled: boolean, size: number): string {
    const template = filled ? HEART_FILLED_SVG : HEART_SVG;
    return template.replaceAll('SIZE', String(size));
}
...
function applyWishState(btn: HTMLButtonElement, wished: boolean, size: number) {
    ...
    if (wished) {
        btn.classList.remove('text-text-tertiary', 'hover:text-wishlist', 'hover:bg-border-light');
        btn.classList.add('text-wishlist');
    } else {
        btn.classList.remove('text-wishlist');
        btn.classList.add('text-text-tertiary', 'hover:text-wishlist', 'hover:bg-border-light');
    }
}

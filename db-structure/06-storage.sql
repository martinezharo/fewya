...
-- imgs bucket: public, for all image uploads (products, profiles, banners, avatars, incidents)
INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES ('imgs', 'imgs', true, 5242880, ARRAY['image/jpeg', 'image/png', 'image/webp', 'image/gif'])
...
-- Storage policies for shop profile and banner images
DROP POLICY IF EXISTS "Allow public read of shop profile images" ON storage.objects;
DROP POLICY IF EXISTS "Allow authenticated users to upload shop profile images" ON storage.objects;
DROP POLICY IF EXISTS "Allow authenticated users to update shop profile images" ON storage.objects;
DROP POLICY IF EXISTS "Allow owners to delete their shop profile images" ON storage.objects;
DROP POLICY IF EXISTS "Allow public read of shop banner images" ON storage.objects;
DROP POLICY IF EXISTS "Allow authenticated users to upload shop banner images" ON storage.objects;
DROP POLICY IF EXISTS "Allow authenticated users to update shop banner images" ON storage.objects;
DROP POLICY IF EXISTS "Allow owners to delete their shop banner images" ON storage.objects;

...
describe('POST /api/orders/resolve-delivery-failure', () => {
    ...
    it('returns 400 when the order is not in delivery_failed status', async () => {
    ...
    it('refunds the full total without a shipping transfer for a full refund', async () => {
        ...
        expect(mockRefundsCreate).toHaveBeenCalledWith(
            ...
            expect.objectContaining({ idempotencyKey: 'delivery-failure-refund-full-4000:convex:ORD-1' }),
        );
    ...
    it('refunds product price and transfers the shipping to the seller for a product refund', async () => {
        ...
        expect(mockTransfersCreate).toHaveBeenCalledWith(
            ...
            expect.objectContaining({ idempotencyKey: 'delivery-failure-shipping-transfer:convex:ORD-1' }),
        );
        expect(await res.json()).toMatchObject({ refundType: 'product', refundedAmount: 35, shippingRetained: 5 });
    });

    it('rejects an unsupported refund type', async () => {
        const res = await call({ orderId: 'convex:ORD-1', refundType: 'partial' });
        expect(res.status).toBe(400);
        expect(mockRefundsCreate).not.toHaveBeenCalled();
    });

    it('returns 500 when the resolution write fails', async () => {
